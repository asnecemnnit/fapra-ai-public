...
model_name = 'transformer_rel'

path_model = 'savedModels/transformer_model_' + model_name
...
transF = TimeSeriesTransformer(input_size = input_dim, dec_seq_len = num_frames_out, batch_first = True,
                               out_seq_len = num_frames_out, num_predicted_features = output_dim).to(device)
optimizer = optim.Adam(transF.parameters(), lr=learning_rate)
...
if synthetic:
    ...
    print("not using synthetic data")
...
for epoch in range(epochs):
    ...
    for batch in data_loader:
        ...
        for i in range(num_frames_in, num_frames_in + num_frames_out):
            y_hat_seq = transF.forward(frame_stack[:, i - num_frames_in : i - num_frames_out, :],
                                       frame_stack[:, i - num_frames_out - 1 : i, :])
            # y_hat_seq = transF.forward(frame_stack[:, i - num_frames_in: i - 5, :])

            frame_stack[:, i, :] = y_hat_seq[:,-1,:]
        ...
        if epoch == 0:  # loss for using the input as output
            ...
            loss_naive = loss_naive + loss_distance_naive.item()
        loss, loss_distance = loss_mse(y, y_hat)
        epoch_loss_train = epoch_loss_train + loss_distance.item()
        torch.autograd.set_detect_anomaly(True)
        optimizer.zero_grad()
        loss.backward()
        #torch.nn.utils.clip_grad_norm_(lstm.parameters(), 1.0)
        optimizer.step()
    epoch_loss_train = epoch_loss_train / len(data_loader)
    if epoch == 0:
        loss_naive = loss_naive / len(data_loader)
    ...
    with torch.no_grad():
        ...
        for batch in data_loader_val:
            ...
            for i in range(num_frames_in, num_frames_in + num_frames_out):
                # y_hat_seq = transF.forward(frame_stack[:, i - num_frames_in : i, :],
                #                            frame_stack[:, i - 1 : i - 1 + num_frames_out, :])
                y_hat_seq = transF.forward(frame_stack[:, i - num_frames_in: i - num_frames_out, :],
                                           frame_stack[:, i - num_frames_out - 1: i, :])
                frame_stack[:, i, :] = y_hat_seq[:,-1,:]
            ...
            loss, loss_distance = loss_mse(y, y_hat)
            epoch_loss_validation = epoch_loss_validation + loss_distance.item()

        epoch_loss_validation = epoch_loss_validation / len(data_loader_val)
    ...
    torch.save(transF, f"{path_model}_in{num_frames_in}_out{num_frames_out}_ts{time_sampling}_ds{data_sampling}"
...
plt.savefig('losses_training_transformer.png')
plt.figure(2)
plt.plot(t, val_losses)

plt.savefig('losses_validation_transformer.png')
